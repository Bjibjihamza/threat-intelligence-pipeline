...
# -*- coding: utf-8 -*-
# =============================================================================
# src/batch/transform/EDA_bronze_to_silver.py
# BRONZE ➜ SILVER: EDA & CLEANING (Batch Processing)
# =============================================================================
# UPDATED: Uses unified Silver loader from src/pipeline/load/load_silver_layer.py
#
# - Loads from raw.cve_details
# - Performs EDA (overview, missing, duplicates, dates, CVSS, products, categories)
# - Cleaning & standardization
# - Maps CWE raw (bronze.category) -> normalized class 'vulnarbilit'
# - Output (no title/description/url/predicted_category):
...
from pipeline.load.load_silver_layer import load_silver_layer
...
LOG_FILE = LOGS_DIR / "eda_bronze_to_silver_batch.log"
...
logger = logging.getLogger("eda_bronze_to_silver_batch")
...
# -------------------------------------------------------------------
# CWE -> classe 'vulnarbilit' (Heuristic Mapping)
# -------------------------------------------------------------------
...
def cwe_to_vulnarbilit(cwe_code: str) -> str:
    """
    Heuristic mapping: 'CWE-xxx' -> normalized class 'vulnarbilit'.
    """
...
# -------------------------------------------------------------------
# EDA (preserves all sections)
# -------------------------------------------------------------------
def perform_eda(df: pd.DataFrame) -> pd.DataFrame:
    """Exploratory Data Analysis with comprehensive logging."""
    logger.info("=" * 72)
    ...
    if 'published_date' in df.columns:
        ...
        if valid_dates > 0:
            logger.info(
                f"   Date range: {df['published_date_parsed'].min()} "
                f"to {df['published_date_parsed'].max()}"
            )
    ...
    if 'cvss_scores' in df.columns:
        ...
        logger.info(
            f"   CVEs with CVSS scores: {has_cvss.sum():,} "
            f"({has_cvss.mean()*100:.2f}%)"
        )
    ...
    if 'affected_products' in df.columns:
        ...
        logger.info(
            f"   CVEs with affected products: {has_products.sum():,} "
            f"({has_products.mean()*100:.2f}%)"
        )

    # Category (bronze) snapshot
...
def clean_silver_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ['published_date', 'last_modified']:
        if col in df.columns:
            df[col] = pd.to_datetime(
                df[col].apply(_parse_date_safe),
                errors='coerce'
            )

    # 3) loaded_at
    if 'loaded_at' in df.columns:
        df['loaded_at'] = pd.to_datetime(
            df['loaded_at'],
            errors='coerce'
        ).dt.tz_localize(None)
    else:
    ...
    date_removed = before_dates - len(df)
    if date_removed > 0:
    ...
    if 'cvss_scores' in df.columns:
        ...
        before_cvss = len(df)
        df = df[has_cvss].copy()
        cvss_removed = before_cvss - len(df)
        if cvss_removed > 0:
...
# -------------------------------------------------------------------
# Build Silver output (aligned with schema)
# -------------------------------------------------------------------
def create_silver_layer(df: pd.DataFrame) -> pd.DataFrame:
    """Create Silver layer with standardized columns."""
    logger.info("=" * 72)
...
def run_eda_to_silver(
    ...
    """
    Complete pipeline: Bronze → EDA → Cleaning → Silver.
    Uses UNIFIED Silver loader.
    """
    logger.info("=" * 72)
    logger.info("🚀 BRONZE ➜ SILVER PIPELINE (BATCH, UNIFIED LOADER)")
    logger.info("=" * 72)
    ...
    try:
        ...
        # Load to Silver (UNIFIED LOADER)
        logger.info("\n💾 Loading to Silver layer (unified loader)...")
        ok = load_silver_layer(
            {"cve_cleaned": silver_df},
            engine,
            if_exists=if_exists,
            convert_jsonb=False
        )
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Bronze ➜ Silver: EDA & Cleaning (Batch, unified loader)"
    )
    p.add_argument(
        '--limit',
        type=int,
        default=None,
        help='Limit rows (test mode)'
    )
    p.add_argument(
        '--if-exists',
